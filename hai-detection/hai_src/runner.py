#!/usr/bin/env python3
"""CLI runner for HAI candidate detection.

Usage:
    python -m hai-detection.src.runner --once
    python -m hai-detection.src.runner --once --dry-run
    python -m hai-detection.src.runner  # Continuous mode
"""
...
from .monitor import HAIMonitor
...
def run_daemon(monitor: HAIMonitor, interval: int | None = None) -> None:
    """Run continuous monitoring."""
    monitor.run_continuous(interval_seconds=interval)


def show_stats(monitor: HAIMonitor) -> None:
    ...
    print(f"Rejected:                 {stats['rejected_hai']}")
    print()


def show_recent(monitor: HAIMonitor, limit: int = 10) -> None:
...
def run_full_pipeline(monitor: HAIMonitor, dry_run: bool = False) -> dict:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="HAI candidate detection and classification",
...
Examples:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Path to HAI database (default: {Config.HAI_DB_PATH})",
    )
    ...
    if args.db_path:
        Config.HAI_DB_PATH = args.db_path

    # Create monitor
    try:
        monitor = HAIMonitor(lookback_hours=args.lookback)
